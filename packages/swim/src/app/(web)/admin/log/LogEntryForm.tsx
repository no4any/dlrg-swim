...
export default function LogEntryForm({ swimmer, onLog, night }: LogEntryFormProps) {
    const [laps, setLaps] = useState<number>(0);

    async function log() {
        onLog(await logAction(swimmer._id?.toString() || "", laps, night), laps);
    }
    ...
    return <div>
        ...
        <div>
            <Button disabled={laps <= 0} onClick={log}>Bahnen erfassen</Button>
        </div>
    </div>
}